...
import RxSwift
import Result
// （3）下面就是本文的重头戏了我们创建一个ViewModel，它的作用就是讲用户各种输入行为，转换成输出状态。本样例中，不管输入还是输出都是Observable类型。
class ViewModel {
    ...
    init(searchAction:Observable<String>) {
        ...
        self.searchResult = searchAction
            ...
            .flatMapLatest{ // 也可以考虑改用flatMapFirst
                GitHubProvider.rx.request(.reposittories($0))
            ...
            }.share(replay:1)// 让http请求是不被共享的
        
        // 生成清空结果动作序列
        self.cleanResult = searchAction.filter{ $0.isEmpty }.map{ _ in Void() }
        
        // 生成查询结果里的许愿列表序列(如果查询到结果则返回结果，如果是清空数据则返回数组)
