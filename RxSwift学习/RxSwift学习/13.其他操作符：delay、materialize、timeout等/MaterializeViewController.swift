//
//  MaterializeViewController.swift
//  RxSwift学习
//
//  Created by bdb on 1/8/19.
...
class MaterializeViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // 十四、其他一些实用的操作符(Observable Utility Opreators)
        /*
         1、delay
         .h该操作符会将Observable的所有元素都先拖延一段设定好的时间，然后才将它们发送出来
         */
        ...
        /*
         2、delaySubscription
         .使用该操作符可以进行拖延时间。即经过所设定的时间后，才对Observable进行订阅操作
         */
        Observable.of(1,2,1)
        ...
        /*
         ...
         .通常一个有限的Observable将产生零个或者多个onNext事件，最后产生一个onCompleted或者onError事件。而materialize操作符会将Observable产生的这些事件全部转换成元素，然后发送出来
         */
        Observable.of(1,2,1)
        .materialize()
        .subscribe(onNext: {print($0)})
        ...
        /*
         4、dematerialize
         .操作符的作用和materialize正好相反，它可以将materialize转换后的元素还原
         */
        Observable.of(1,2,1)
        .materialize()
        .dematerialize()
        ...
        /*
         5、timeout
         .使用该操作符可以设置一个超时时间。如果Observable在规定内没有发任何出元素，就产生一个超时的error事件
         */
        // 定义好每个事件里的值以及发送的时间
//        let times = [
//            [ "value": 1, "time": 0 ],
...
//        .disposed(by: disposeBag)
        ...
        /*
         6、使用using操作符创建Observable时，同时会创建一个可被清除的资源，一旦Observable终止了，那么这个资源就会被清除掉了
         */
        // 一个无线序列(每隔0.1秒创建一个序列数)
